...
- [medium] → `work-cards/15-doe-guidance-line.md` — DOE guidance line under the reading, **unblocked by the builder's option-1 sourcing decision** (2026-09-15): ships citing the National Haze Action Plan — outdoor activities suspended above API 100, schools close above API 200 (the API 150/NADMA trigger stays in the card's sourcing note, not the UI) — with the mandatory Malaysian-API-vs-US-AQI scale caveat; agent-side fetches were bot-blocked, verification chain builder-attested (RTM quoting DOE Deputy DG + corroborating outlets) and logged in the card (promoted 2026-09-15 — builder priority 3 of 3)
